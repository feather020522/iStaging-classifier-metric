...
class CustomZinDBuildingParser(Dataset):
    def __init__(self, buildings, data_dir, transform, filter = False):
    ...
    def __getitem__(self, idx):
        ...
        return panos, new_labels
            

def PanoLabelGetter(house_data, partition_data, train_type, filter=0):
    ...
    for id, houses in house_data.items():
            if id in partition_data[train_type]:
                ...
                # if i < 15:
                    ...
                    # print(id, i, j)
                # else:
                # print(f'{id}: {i}', file=fd)
                if filter and i > filter:
                    continue
                building_room_pairs.append(room_pairs)

    return label_pairs, building_room_pairs
...
train_panos, train_buildings = PanoLabelGetter(houses_data, partition_data, "train")               # total: 67448 panos, 1575 buildings
val_panos, val_buildings = PanoLabelGetter(houses_data, partition_data, "val", filter_size)
test_panos, test_buildings = PanoLabelGetter(houses_data, partition_data, "test", filter_size)
